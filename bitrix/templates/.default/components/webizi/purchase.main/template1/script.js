...
WI.purchase = {
	...
	purTamplate: function(data) {
		...
		for (key in data) {
			...
			+ '<div class="purchase_item_date">Опубликовано: <br><span>' + data[key]["PUBL_DATE"] + '</span></div>'
			+ '<div class="purchase_item_date purchase_item_marg">Дата начала подачи заявок:<br><span>' + data[key]["DFROM"] + '</span></div>'
			+ '<div class="purchase_item_date purchase_item_marg">Дата окончания подачи заявок:<br><span>' + data[key]["DTO"] + ' МСК</span></div>';
			if (data[key]["PURCHASE_DATE"]){
				element += '<div class="purchase_item_date purchase_item_marg">Дата проведения закупок:<br><span>' + data[key]["PURCHASE_DATE"] + '</span></div>'
			} 
			element += '</div>'
