...
{	
	...
	if(is_array($arPost['NAMES']))
	{
		foreach($arPost['NAMES'] as $propIndex)
		{
			...
			{
				if(strlen($arPropFields['CODE']) > 0)
				{
					$index = 0;
					while(($dbRes2 = CIBlockProperty::GetList(array(), array('CODE'=>$arPropFields['CODE'], 'IBLOCK_ID'=>$arPropFields['IBLOCK_ID']))) && ($arr2 = $dbRes2->Fetch()))
					{
						$index++;
