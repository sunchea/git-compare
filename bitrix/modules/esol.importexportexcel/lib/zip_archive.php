...
class ZipArchive
{
	...
	public static function RemoveFileDir($dir)
	{
		...
		elseif(is_numeric($dir)) $dir = $_SERVER["DOCUMENT_ROOT"].'/upload/tmp/'.static::$moduleId.'/'.static::$moduleSubDir.'_archives/'.$dir.'/';
		if($dir && is_dir($dir))
		{
	...
	public static function GetFileDir($pFilename)
	{
		if(($pos = strpos($pFilename, '/'.static::$moduleId.'/'))!==false)
		{
			...
			{
				return $_SERVER["DOCUMENT_ROOT"].'/upload/tmp/'.static::$moduleId.'/'.static::$moduleSubDir.'_archives/'.$arFile['ID'].'/';
			}
		}
		return false;
	}
