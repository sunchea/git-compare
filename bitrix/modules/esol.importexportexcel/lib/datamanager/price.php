...
class Price
{
	...
	public function SavePrice($ID, $arPrices, $isOffer = false)
	{
		...
		foreach($arPrices as $gid=>$arFieldsPrice)
		{
			...
			{
				if(isset($arFieldsPrice['PRICE']) || isset($arFieldsPrice['CURRENCY']))
				{
			...
			foreach($arSubPrices as $arSubPrice)
			{
				...
				if(isset($arFieldsPrice['PRICE']) && $arFieldsPrice['PRICE']!=='-') $arFieldsPrice['PRICE'] = $this->GetFloatVal($arFieldsPrice['PRICE']);
