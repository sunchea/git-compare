				...
				foreach($arSections as $arSection){
					if($arSection["IBLOCK_SECTION_ID"] == $PSID){
						...
						if($arItem[3]["IS_PARENT"]){
							...
							// section elements
							if(is_array($arItemsBySectionID[$arSection["ID"]])){
								foreach($arItemsBySectionID[$arSection["ID"]] as $arItem){
									$aMenuLinksExt[] = array($arItem["NAME"], (!is_array($arItem["DETAIL_PAGE_URL"]) ? $arItem["DETAIL_PAGE_URL"] : $arItem["DETAIL_PAGE_URL"][$arSection["ID"]]), array(), array("FROM_IBLOCK" => 1, "DEPTH_LEVEL" => ($arSection["DEPTH_LEVEL"] + 1), "IS_ITEM" => 1));
								}
							}
						}
					}
