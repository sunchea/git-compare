...
<script>
	<?require_once($_SERVER['DOCUMENT_ROOT']."/bitrix/wizards/aspro/allcorp/js/jquery-1.8.3.min.js");?>
	<?require_once($_SERVER['DOCUMENT_ROOT']."/bitrix/wizards/aspro/allcorp/js/jquery.keyboard.js");?>
	function setWizardBackgroundColor(theme){
		window.console&&console.log(theme);
		switch (theme) {
			case "color1":
				$(".instal-bg").css("backgroundColor", "#c21f13");
				break;
			case "color2":
			...
			case "color10":
				...
				break;
			case "color11":
				$(".instal-bg").css("backgroundColor", "#color11");
				break;
			case "color12":
...
class SelectSiteStep extends CSelectSiteWizardStep{
	function InitStep(){
		...
		$wizard->solutionName = "allcorp";
		require_once($_SERVER["DOCUMENT_ROOT"]."/bitrix/wizards/aspro/allcorp/css/styles.css");
	}
}
...
class SelectTemplateStep extends CSelectTemplateWizardStep{
	function InitStep(){
		...
		$wizard->SetDefaultVars(Array("templateID" => "allcorp"));
	}
	...
	function ShowStep(){
		if (!CModule::IncludeModule("aspro.allcorp")){
			$this->content .= "<p style='color:red'>".GetMessage("WIZ_NO_MODULE_ALLCORP")."</p>";
			?>
...
class SelectThemeStep extends CSelectThemeWizardStep{
	...
	function ShowStep(){
		...
		if(!strlen($ThemeID)){
			if(!strlen($ThemeID = COption::GetOptionString("aspro.allcorp", "COLOR", "", $siteID))){
				$ThemeID = "color7";
			}
		}
		...
		foreach($arThemes as $themeID => $arTheme){
			...
			$this->content .= '
				<span themeName="'.$themeID.'" class="inst-template-color'.($defaultThemeID == $themeID ? " inst-template-color-selected" : "").'" ondblclick="SubmitForm(\'next\');"  onclick="SelectTheme(this, \''.$themeID.'\', \''.$arTheme["SCREENSHOT"].'\');">
					<span class="inst-templ-color-img">'.CFile::ShowImage($arTheme["PREVIEW"], 70, 54, ' border="0" class="solution-image"').'</span>
			...
			'<div id="solution-preview">'.
				'<b class="r3"></b><b class="r1"></b><b class="r1"></b>'.
					'<div class="solution-inner-item">'.
						CFile::ShowImage($arDefaultTheme["SCREENSHOT"], 450, 686, ' border="0" id="solution-preview-image"').
					'</div>'.
...
class SiteSettingsStep extends CSiteSettingsWizardStep{
	function InitStep(){
		...
		$wizard->SetDefaultVars(
			Array(
				...
				"siteTelephone" => $this->GetFileContent(WIZARD_SITE_PATH."include/site-phone.php", GetMessage("WIZ_COMPANY_TELEPHONE_DEF")),
				"siteCopy" => $this->GetFileContent(WIZARD_SITE_PATH."include/copy.php", GetMessage("WIZ_COMPANY_COPY_DEF")),
				"siteEmail" => strip_tags($this->GetFileContent(WIZARD_SITE_PATH."include/site-email.php", GetMessage("WIZ_COMPANY_EMAIL_DEF"))),
				"siteSkype" => $this->GetFileContent(WIZARD_SITE_PATH."include/site-skype.php", GetMessage("WIZ_COMPANY_SKYPE_DEF")),
				"siteAddress" => $this->GetFileContent(WIZARD_SITE_PATH."include/site-address.php", GetMessage("WIZ_COMPANY_ADDRESS_DEF")),
				"siteSchedule" => $this-> GetFileContent(WIZARD_SITE_PATH."include/site-schedule.php", GetMessage("WIZ_COMPANY_SCHEDULE_DEF")),
				...
				"shopOdnoklassniki" => GetMessage("WIZ_SHOP_ODNOKLASSNIKI_DEF"),
				"shopMailru" => GetMessage("WIZ_SHOP_MAILRU_DEF"),
				"shopLiveJournal" => GetMessage("WIZ_SHOP_LIVEJOURNAL_DEF"),
				"shopGoogle" => GetMessage("WIZ_SHOP_GOOGLE_DEF"),
	...
	function ShowStep(){
		require_once($_SERVER["DOCUMENT_ROOT"]."/bitrix/wizards/aspro/allcorp/css/styles_rewrite.css");
		$wizard =& $this->GetWizard();
		$templateID = $wizard->GetVar("templateID");
		$themeVarName = $templateID."_themeID";
		$themeID = $wizard->GetVar($themeVarName);
...
class DataInstallStep extends CDataInstallWizardStep{
	...
	function CorrectServices(&$arServices){	
		if($_SESSION["BX_allcorp_LOCATION"] == "Y") $this->repeatCurrentService = true;
...
class FinishStep extends CFinishWizardStep{
	...
	function ShowStep(){
		...
		if(CModule::IncludeModule("aspro.allcorp")) {
			CAllCorp::newAction("wizard_installed");
		}
		
		COption::SetOptionString("aspro.allcorp", "WIZARD_DEMO_INSTALLED", "Y");
	}
}
?>