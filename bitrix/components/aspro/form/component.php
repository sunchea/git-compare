	...
	foreach( $arResult["QUESTIONS"] as $FIELD_CODE => $arQuestion ){
					...
					foreach($_FILES as $key => $arFile){
						...
						if($code[0] == $FIELD_CODE){
							if(strlen($arFile['name'])){
								...
								$val .= (strlen($val) ? ', ' : '').$arFile['name'].' ('.($arFile['size'] < 1024 ? $arFile['size'].GetMessage("S_B") : (floor($arFile['size'] / 1024) < 1024 ? number_format($arFile['size'] / 1024, 1, '.', ' ').GetMessage("S_KB") : number_format($arFile['size'] / 1048576, 1, '.', ' ').GetMessage("S_MB"))).')';
							}
						}
					}
				}
		...
		switch( $arQuestion["FIELD_TYPE"] ){
			...
			case "integer":
				...
				break;
			case "date":
				$html = '<input type="date" id="'.$arQuestion["CODE"].'" name="'.$arQuestion["CODE"].'" class="form-control date '.$required.'" '.$placeholder.' value="'.$val.'" />'.$icon;
				break;
			case "html":
			...
			case "file":
				if( $arQuestion["MULTIPLE"] == "Y" ){
					$html = '';
					for( $i = 0; $i < 4; ++$i ){
						$html .= '<input type="file" id="'.$arQuestion["CODE"].'" name="'.$arQuestion["CODE"].'_n'.$i.'" '.$required.' '.$placeholder.' value="'.$valFile['n'.$i].'" />'.$icon;
					}
				}else{
					$html = '<input type="file" id="'.$arQuestion["CODE"].'" name="'.$arQuestion["CODE"].'" '.$required.' '.$placeholder.' value="'.$valFile.'" />'.$icon;
				}
				break;
		}
				...
				//if( check_bitrix_sessid() ){
					...
					foreach($_REQUEST as $code => $value){
						...
						elseif($arResult["QUESTIONS"][$code]["FIELD_TYPE"] == "date"){
							$_REQUEST[$code] = array("VALUE" => str_replace(array('-', '/', ' ', ':'), array('.', '.', '.', '.'), $arQuestion['VALUE']));
						}
					}
