...
if(WIZARD_INSTALL_DEMO_DATA){
	...
	CUserOptions::SetOption("form", "form_element_".$iblockID, array(
		'tabs' => 'edit1--#--'.GetMessage("WZD_ELEMENT").'--,--ACTIVE--#--'.GetMessage("WZD_OPTION_ACTIVE").'--,--NAME--#--'.GetMessage("WZD_OPTION_NAME_").'--,--SORT--#--'.GetMessage("WZD_OPTION_SORT").'--,--PROPERTY_'.$arProperty["ICON"].'--#--'.GetMessage("WZD_OPTION_PROP_ICON").'--,--PREVIEW_PICTURE--#--'.GetMessage("WZD_OPTION_PREVIEW_PICTURE").'--,--PROPERTY_'.$arProperty["LINK"].'--#--'.GetMessage("WZD_OPTION_PROP_LINK").'--,--PREVIEW_TEXT--#--'.GetMessage("WZD_OPTION_PREVIEW_TEXT").'--;--',
	));
	// list user options
	CUserOptions::SetOption("list", "tbl_iblock_list_".md5($iblockTYPE.".".$iblockID), array(
		'columns' => 'NAME,PROPERTY_'.$arProperty["ICON"].',PREVIEW_PICTURE,PROPERTY_'.$arProperty["LINK"].',ACTIVE,SORT,TIMESTAMP_X,ID', 'by' => 'sort', 'order' => 'asc', 'page_size' => '20', 
	));
}

if ($iblockID){
	// replace macros IBLOCK_TYPE & IBLOCK_ID & IBLOCK_CODE
	CWizardUtil::ReplaceMacrosRecursive($bitrixTemplateDir, Array("IBLOCK_MAINLINKS_TYPE" => $iblockTYPE));
	CWizardUtil::ReplaceMacrosRecursive($bitrixTemplateDir, Array("IBLOCK_MAINLINKS_ID" => $iblockID));
	CWizardUtil::ReplaceMacrosRecursive($bitrixTemplateDir, Array("IBLOCK_MAINLINKS_CODE" => $iblockCODE));
}
?>